...
class QNetwork(nn.Module):
    ...
    def __init__(self, state_size, action_size, hidden_layer_neurons=64, seed=0):
        ...
        self.feature_layer = nn.Linear(state_size, hidden_layer_neurons)
        
        # set advantage layer
        self.advantage_layer1 = nn.Linear(hidden_layer_neurons, hidden_layer_neurons)
        self.advantage_layer2 = nn.Linear(hidden_layer_neurons, action_size)

        # set value layer
        self.value_layer1 = nn.Linear(hidden_layer_neurons, hidden_layer_neurons)
        self.value_layer2 = nn.Linear(hidden_layer_neurons, 1)
        
    def forward(self, state):
        """Forward method implementation."""
        feature = F.relu(self.feature_layer(state))
        
        advantage = F.relu(self.advantage_layer1(feature))
        advantage = self.advantage_layer2(advantage)
        
        value = F.relu(self.value_layer1(feature))
