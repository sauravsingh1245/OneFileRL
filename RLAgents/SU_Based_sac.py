...
class SUBasedSACAgent:
    def __init__(self, observation_shape, action_size, max_action=1.0, seed=0, device='cpu', lr=0.0003, buffer_size=100000, 
                 batch_size=256, gamma=0.99, tau=0.005, hidden_layer_neurons=256, start_steps=10000, updates_per_step=1,
                 target_update_interval=1, alpha=0.2, automatic_entropy_tuning=False,
                 loss_factor = 100.0, target_su=0.5, focus_state=0):
        ...
        if len(self.memory) > self.batch_size:
            ...
            for gradient_step in range(self.updates_per_step):
                ...
                if disable_su_training: 
                    ...
                    # Jπ = 𝔼st∼D,εt∼N[α * logπ(f(εt;st)|st) − Q(st,f(εt;st))]
                    mu_loss_term = F.mse_loss(torch.Tensor([self.target_su]).to(self.device), su[self.focus_state])
                    
                    # mu_loss_term = F.mse_loss(torch.Tensor([0.2]).to(self.device), su[0]) +\
                    #                 F.mse_loss(torch.Tensor([0.25]).to(self.device), su[1]) +\
                    #                 F.mse_loss(torch.Tensor([0.3]).to(self.device), su[2])

                    actor_loss = ((self.alpha * log_pi) - min_qf_pi).mean() + self.loss_factor*mu_loss_term
    ...
    def save_agent(self, filepath):
        torch.save({
            ...
            'Critics': self.critic.state_dict()
        }, filepath)

    def load_agent(self, filepath):
        checkpoint = torch.load(filepath)
        self.actor.load_state_dict(checkpoint['Actor'])
        self.critic.load_state_dict(checkpoint['Critics'])
        self.critic_target.load_state_dict(checkpoint['Critics'])
        self.actor.train()
        self.critic.train()
        self.critic_target.train()


    def compute_KLDivergence(self, mu1, mu2, std1, std2):
    ...
    def feature_importance(self, states):
        ...
        mu_orig, log_std_orig = self.actor(states)
        std_orig = log_std_orig.exp()

        for i in range(M):
            states_i = states.clone()
            states_i[:,i] = states_i[:,i][torch.randperm(N)]
            mu_i, log_std_i = self.actor(states_i)
            std_i = log_std_i.exp()
            kl = self.compute_KLDivergence(mu_orig, mu_i, std_orig, std_i)
            su_score.append(kl)
